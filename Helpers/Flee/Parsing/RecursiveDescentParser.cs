...
internal class RecursiveDescentParser : Parser
{
    ...
    public override void Prepare()
    {
        ...
        while (e.MoveNext()) CalculateLookAhead((ProductionPattern)e.Current);
    ...
    private Node ParsePattern(ProductionPattern pattern)
    {
        ...
        if (_stackdepth > 200) throw new StackOverflowException();
        ...
        try
        {
            ...
            {
                var alt = pattern[i];
                if (defaultAlt != alt && IsNext(alt)) return ParseAlternative(alt);
            }

            if (defaultAlt == null || !IsNext(defaultAlt)) ThrowParseException(FindUnion(pattern));
    ...
    private bool IsNext(ProductionPatternElement elem)
    {
        ...
        if (set != null) return set.IsNext(this);

        if (elem.IsToken()) return elem.IsMatch(PeekToken(0));
    ...
    private void CalculateLookAhead(ProductionPattern pattern)
    {
        ...
        if (pattern.LookAhead == null) pattern.LookAhead = result;
        ...
        for (i = 0; i < pattern.Count; i++) CalculateLookAhead(pattern[i], 0);
    }
        ...
        LookAheadSet filter)
    {
        LookAheadSet follow;
        // Check trivial cases
        if (length <= 0 || pos >= alt.Count) return new LookAheadSet(0);

        // Find look-ahead for this element
        var first = FindLookAhead(alt[pos], length, stack, filter);
        if (alt[pos].MinCount == 0) first.AddEmpty();
        ...
        LookAheadSet filter)
    {
        ...
        var result = new LookAheadSet(length);
        result.AddAll(first);
        if (filter == null || !filter.IsOverlap(result)) return result;
        ...
        for (var i = 1; i < max; i++)
        {
            ...
            if (first.Size() <= 0 || first.GetMinLength() >= length) break;
        ...
        LookAheadSet filter)
    {
        ...
        else
        {
            ...
            if (stack.Contains(pattern.Name)) result = result.CreateRepetitive();
        }
        ...
        int maxLength)
    {
        ...
        if (result.IsRepetitive()) ThrowAmbiguityException(pattern.Name, null, result);
        return result;
    }
        ...
        LookAheadSet set2)
    {
        ...
        if (result.IsRepetitive()) ThrowAmbiguityException(pattern, location, result);
        return result;
    }
    ...
    private LookAheadSet FindUnion(ProductionPattern pattern)
    {
        ...
        for (i = 0; i < pattern.Count; i++)
        {
            ...
            if (result.GetMaxLength() > length) length = result.GetMaxLength();
        }

        result = new LookAheadSet(length);
        for (i = 0; i < pattern.Count; i++) result.AddAll(pattern[i].LookAhead);
    ...
    private void ThrowParseException(LookAheadSet set)
    {
        ...
        while (set.IsNext(this, 1)) set = set.CreateNextSet(NextToken().Id);

        // Find next token descriptions
        var initials = set.GetInitialTokens();
        for (var i = 0; i < initials.Length; i++) list.Add(GetTokenDescription(initials[i]));
    ...
    private class CallStack
    {
        ...
        {
            for (var i = 0; i < _nameStack.Count; i++)
