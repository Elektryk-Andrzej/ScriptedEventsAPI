...
internal class TokenNFA
{
    ...
    public void AddTextMatch(string str, bool ignoreCase, TokenPattern value)
    {
        ...
        if (ch < 128 && !ignoreCase)
        {
            ...
            if (state == null) state = _initialChar[ch] = new NFAState();
        }
        else
        {
        ...
        for (var i = 1; i < str.Length; i++) state = state.AddOut(str[i], ignoreCase, null);
        state.Value = value;
    }
    ...
    public void AddRegExpMatch(string pattern,
        ...
        TokenPattern value)
    {
        var parser = new TokenRegExpParser(pattern, ignoreCase);
        var debug = "DFA regexp; " + parser.GetDebugInfo();

        var isAscii = parser.Start.IsAsciiOutgoing();
        for (var i = 0; isAscii && i < 128; i++)
        {
            var match = false;
            for (var j = 0; j < parser.Start.Outgoing.Length; j++)
            ...
            if (match && _initialChar[i] != null) isAscii = false;
        }
    ...
    public int Match(ReaderBuffer buffer, TokenMatch match)
    {
        ...
        if (0 <= peekChar && peekChar < 128)
        {
            ...
            if (state != null) _queue.AddLast(state);
        }

        if (peekChar >= 0) _initial.MatchTransitions((char)peekChar, _queue, true);
        _queue.MarkEnd();
        ...
        while (!_queue.Empty)
        {
            ...
            if (state.Value != null) match.Update(pos, state.Value);
            if (peekChar >= 0) state.MatchTransitions((char)peekChar, _queue, false);
        }
...
internal class NFAState
{
    ...
    public NFAState AddOut(char ch, bool ignoreCase, NFAState state)
    {
        if (ignoreCase)
        {
            if (state == null) state = new NFAState();
    ...
    public NFAState AddOut(NFATransition trans)
    {
        ...
        if (trans is NFAEpsilonTransition) EpsilonOut = true;
        return trans.State;
    }
    ...
    public void MergeInto(NFAState state)
    {
        ...
        Incoming = null;
        for (var i = 0; i < Outgoing.Length; i++) state.AddOut(Outgoing[i]);
        Outgoing = null;
    }
    ...
    private NFAState FindUniqueCharTransition(char ch)
    {
        ...
        {
            trans = Outgoing[i];
            if (trans != res && trans.State == res.State) return null;
        }
    ...
    public void MatchTransitions(char ch, NFAStateQueue queue, bool initial)
    {
        for (var i = 0; i < Outgoing.Length; i++)
        {
            ...
            else if (trans.Match(ch))
            {
                ...
                if (target.EpsilonOut) target.MatchEmpty(queue);
            }
        }
    }
...
internal class NFACharRangeTransition : NFATransition
{
    ...
    public override bool IsAscii()
    {
        ...
        for (var i = 0; i < _contents.Length; i++)
        {
            ...
            {
                var c = (char)obj;
                if (c < 0 || 128 <= c) return false;
            }
            else if (obj is Range)
            {
                if (!((Range)obj).IsAscii()) return false;
            }
        }
    ...
    public override bool Match(char ch)
    {
        ...
        if (IgnoreCase) ch = char.ToLower(ch);
        for (var i = 0; i < _contents.Length; i++)
        {
            obj = _contents[i];
            if (obj is char)
            {
                c = (char)obj;
                if (c == ch) return !Inverse;
            }
            else if (obj is Range)
            {
                r = (Range)obj;
                if (r.Inside(ch)) return !Inverse;
            }
        }
