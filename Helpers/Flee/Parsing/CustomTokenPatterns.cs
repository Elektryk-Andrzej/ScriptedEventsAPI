...
internal abstract class CustomTokenPattern : TokenPattern
{
    ...
    protected abstract void ComputeToken(int id, string name, PatternType type, string pattern,
...
internal class RealPattern : CustomTokenPattern
{
    ...
    protected override void ComputeToken(int id, string name, PatternType type, string pattern,
        ExpressionContext context)
    {
        var options = context.ParserOptions;

        var digitsBeforePattern = options.RequireDigitsBeforeDecimalPoint ? '+' : '*';
...
{
    public ArgumentSeparatorPattern(int id, string name, PatternType type, string pattern) : base(id, name, type,
        pattern)
    {
    }
