...
internal class CharacterSetElement : Element
{
    ...
    {
        for (var i = 0; i < str.Length; i++) AddCharacter(str[i]);
    }
        ...
        int skip)
    {
        ...
        if (m.IsCaseInsensitive()) c = char.ToLower((char)c);
        return InSet((char)c) ? 1 : -1;
    }

    private bool InSet(char c)
    {
        if (this == Dot) return InDotSet(c);

        if (this == Digit || this == NonDigit) return InDigitSet(c) != _inverted;

        if (this == Whitespace || this == NonWhitespace) return InWhitespaceSet(c) != _inverted;

        if (this == Word || this == NonWord) return InWordSet(c) != _inverted;
    ...
    public override string ToString()
    {
        ...
        if (this == Digit) return "\\d";

        if (this == NonDigit) return "\\D";

        if (this == Whitespace) return "\\s";

        if (this == NonWhitespace) return "\\S";

        if (this == Word) return "\\w";

        if (this == NonWord) return "\\W";
        ...
        if (_inverted)
            ...
            buffer.Append("[");
        for (var i = 0; i < _contents.Count; i++) buffer.Append(_contents[i]);
        buffer.Append("]");
