...
internal class TokenRegExpParser
{
    ...
    {
        if (_stateCount == 0) UpdateStats(Start, new Hashtable());
    ...
    private void UpdateStats(NFAState state, Hashtable visited)
    {
        if (!visited.ContainsKey(state))
        {
            ...
            {
                _transitionCount++;
                if (state.Outgoing[i] is NFAEpsilonTransition) _epsilonCount++;
    ...
    private NFAState ParseAtomModifier(NFAState start, NFAState end)
    {
        ...
        if (min == 0 && max == 1) return start.AddOut(new NFAEpsilonTransition(end));
    ...
    private char ReadEscapeChar()
    {
        ...
        switch (c)
        {
            case '0':
                ...
                return (char)value;
            case 'x':
                str = ReadChar().ToString()  + ReadChar();
                try
                {
    ...
    {
        if (_pos + count < _pattern.Length) return _pattern[_pos + count];
