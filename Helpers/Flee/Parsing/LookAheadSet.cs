...
internal class LookAheadSet
{
    ...
    public int GetMinLength()
    {
        ...
        for (var i = 0; i < _elements.Count; i++)
        {
            ...
            if (min < 0 || seq.Length() < min) min = seq.Length();
        }
    ...
    public int[] GetInitialTokens()
    {
        ...
        for (i = 0; i < list.Count; i++) result[i] = (int)list[i];
        return result;
    }
    ...
    public bool IsRepetitive()
    {
        ...
        {
            var seq = (Sequence)_elements[i];
            if (seq.IsRepetitive()) return true;
        }
    ...
    public bool IsNext(Parser parser)
    {
        ...
        {
            var seq = (Sequence)_elements[i];
            if (seq.IsNext(parser)) return true;
        }
    ...
    public bool IsNext(Parser parser, int length)
    {
        ...
        {
            var seq = (Sequence)_elements[i];
            if (seq.IsNext(parser, length)) return true;
        }
    ...
    private bool IsOverlap(Sequence seq)
    {
        for (var i = 0; i < _elements.Count; i++)
        {
            ...
            if (seq.StartsWith(elem) || elem.StartsWith(seq)) return true;
        }
    ...
    {
        if (seq.Length() > _maxLength) seq = new Sequence(_maxLength, seq);
        if (!Contains(seq)) _elements.Add(seq);
    }
    ...
    {
        for (var i = 0; i < set._elements.Count; i++) Add((Sequence)set._elements[i]);
    }
    ...
    {
        for (var i = 0; i < set._elements.Count; i++) Remove((Sequence)set._elements[i]);
    }

    public LookAheadSet CreateNextSet(int token)
    {
        var result = new LookAheadSet(_maxLength - 1);
        for (var i = 0; i < _elements.Count; i++)
        {
            var seq = (Sequence)_elements[i];
            var value = seq.GetToken(0);
            if (value != null && token == (int)value) result.Add(seq.Subsequence(1));
        }
    ...
    {
        var result = new LookAheadSet(_maxLength);
        for (var i = 0; i < _elements.Count; i++)
        {
            ...
            if (seq2 != null && seq1.IsRepetitive())
                result.Add(seq2);
            else if (seq2 != null) result.Add(seq1);
        }
    ...
    public LookAheadSet CreateCombination(LookAheadSet set)
    {
        ...
        for (var i = 0; i < _elements.Count; i++)
        {
            ...
            if (first.Length() >= _maxLength)
                ...
                for (var j = 0; j < set._elements.Count; j++)
                {
    ...
    public LookAheadSet CreateOverlaps(LookAheadSet set)
    {
        ...
        {
            var seq = (Sequence)_elements[i];
            if (set.IsOverlap(seq)) result.Add(seq);
        }
    ...
    public LookAheadSet CreateFilter(LookAheadSet set)
    {
        ...
        // Handle special cases
        if (Size() <= 0 || set.Size() <= 0) return this;
        ...
        for (var i = 0; i < _elements.Count; i++)
        {
            ...
            {
                var second = (Sequence)set._elements[j];
                if (first.StartsWith(second)) result.Add(first.Subsequence(second.Length()));
            }
        }
    ...
    private class Sequence
    {
        ...
        public Sequence(int length, Sequence seq)
        {
            ...
            if (seq.Length() < length) length = seq.Length();
            for (var i = 0; i < length; i++) _tokens.Add(seq._tokens[i]);
        }
        ...
        {
            if (pos >= 0 && pos < _tokens.Count) return _tokens[pos];
        ...
        {
            if (obj is Sequence) return Equals((Sequence)obj);
        ...
        {
            if (_tokens.Count != seq._tokens.Count) return false;
        ...
        public bool StartsWith(Sequence seq)
        {
            ...
            for (var i = 0; i < seq._tokens.Count; i++)
                if (!_tokens[i].Equals(seq._tokens[i]))
                    return false;
        ...
        public bool IsNext(Parser parser)
        {
            for (var i = 0; i < _tokens.Count; i++)
            {
                ...
                if (token == null || token.Id != id) return false;
            }
        ...
        public bool IsNext(Parser parser, int length)
        {
            if (length > _tokens.Count) length = _tokens.Count;
        ...
        public Sequence Concat(int length, Sequence seq)
        {
            ...
            if (seq._repeat) res._repeat = true;
            length -= Length();
