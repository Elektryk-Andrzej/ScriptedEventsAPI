...
internal class RepeatElement : Element
{
        ...
        int skip)
    {
        ...
        switch (_type)
        {
            ...
            case RepeatType.POSSESSIVE:
                if (skip == 0) return MatchPossessive(m, buffer, start, 0);
                break;
        }
        ...
        int count)
    {
        ...
        // Return result
        if (_min <= count && count <= _max) return length;
        ...
        int attempt)
    {
        ...
        {
            if (_matches.Length <= length) _matches.Length = length + 10;
    ...
    public override void PrintTo(TextWriter output, string indent)
    {
        ...
        if (_type == RepeatType.RELUCTANT)
            output.Write("?");
        else if (_type == RepeatType.POSSESSIVE) output.Write("+");
        output.WriteLine();
