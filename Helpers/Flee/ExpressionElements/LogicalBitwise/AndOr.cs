...
internal class AndOrElement : BinaryExpressionElement
{
    ...
    protected override Type GetResultType(Type leftType, Type rightType)
    {
        ...
        if (bitwiseOpType != null) return bitwiseOpType;
    ...
    ///     the stack of operators (ignoring siblings) until we find a different operation (then emit a branch to its right
    ///     operand)
    ...
    private static void EmitLogicalShortCircuit(FleeILGenerator ilg, ShortCircuitInfo info, IServiceProvider services)
    {
        while (info.Operators.Count != 0)
        {
            ...
            var l = GetShortCircuitLabel(op, info, ilg);
            // Emit the branch
    ...
    private static Label GetShortCircuitLabel(AndOrElement current, ShortCircuitInfo info, FleeILGenerator ilg)
    {
        ...
        if (current._myOperation == AndOrOperation.And) return GetLabel(OurFalseTerminalKey, ilg, info);
    ...
    {
        var andOrChild = MyRightChild as AndOrElement;
    ...
    private void Pop(Stack operands, Stack operators)
    {
        ...
        var andOrChild = MyLeftChild as AndOrElement;
        if (andOrChild == null)
            operands.Pop();
        else
            andOrChild.Pop(operands, operators);
        ...
        IServiceProvider services)
    {
        ...
        {
            // Yes, so mark it
            var leftLabel = info.FindLabel(operand);
    ...
    private static void EmitTerminals(ShortCircuitInfo info, FleeILGenerator ilg, Label endLabel)
    {
        ...
        {
            var falseLabel = info.FindLabel(OurFalseTerminalKey);
        ...
        if (info.HasLabel(OurTrueTerminalKey))
        {
            var trueLabel = info.FindLabel(OurTrueTerminalKey);
