...
internal class ShiftElement : BinaryExpressionElement
{
    ...
    protected override Type GetResultType(Type leftType, Type rightType)
    {
        ...
        if (ImplicitConverter.EmitImplicitNumericConvert(rightType, typeof(int), null) == false) return null;

        // Left argument must be an integer type
        if (Utility.IsIntegralType(leftType) == false) return null;

        var tc = Type.GetTypeCode(leftType);
    ...
    private void EmitShiftCount(FleeILGenerator ilg, IServiceProvider services)
    {
        ...
        var tc = Type.GetTypeCode(MyLeftChild.ResultType);
        switch (tc)
        {
            case TypeCode.Byte:
            case TypeCode.SByte:
