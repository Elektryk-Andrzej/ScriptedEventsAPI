...
internal class InvocationListElement : ExpressionElement
{
    ...
    private static void LinkElements(IList elements)
    {
        for (var i = 0; i <= elements.Count - 1; i++)
        {
            ...
            if (i + 1 < elements.Count) nextElement = (MemberElement)elements[i + 1];
    ...
    private void ResolveNamespaces(IList elements, IServiceProvider services)
    {
        ...
        while (true)
        {
            ...
            if (name == null) break; // TODO: might not be correct. Was : Exit While

            var import = currentImport.FindImport(name);

            if (import == null) break; // TODO: might not be correct. Was : Exit While
        ...
        if (elements.Count == 0)
            ThrowCompileException(CompileErrorResourceKeys.NamespaceCannotBeUsedAsType,
    ...
    {
        foreach (MemberElement element in elements) element.Resolve(services);
    }
