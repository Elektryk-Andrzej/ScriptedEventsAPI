...
internal abstract class CustomBinder : Binder
{
    ...
    public MethodBase BindToMethod(BindingFlags bindingAttr, MethodBase[] match, ref object[] args,
        ParameterModifier[] modifiers, CultureInfo culture, string[] names, ref object state)
    {
        return null;
    }
...
internal class ExplicitOperatorMethodBinder : CustomBinder
{
    ...
    {
        return _customBinderImplementation.BindToMethod(bindingAttr, match, ref args, modifiers, culture, names,
            out state);
    }
        ...
        ParameterModifier[] modifiers)
    {
        foreach (MethodInfo mi in match)
        {
            ...
            if (ReferenceEquals(firstParameter.ParameterType, _myArgType) &
                ReferenceEquals(mi.ReturnType, _myReturnType)) return mi;
        }
...
internal class BinaryOperatorBinder : CustomBinder
{
        ...
        ParameterModifier[] modifiers)
    {
        foreach (MethodInfo mi in match)
        {
            ...
            if (leftValid & rightValid) return mi;
        }
...
internal class CustomMethodInfo : IComparable<CustomMethodInfo>, IEquatable<CustomMethodInfo>
{
    ...
    public void ComputeScore(Type[] argTypes)
    {
        ...
        if (@params.Length == 0)
            ...
            (@params.Length == 1 && argTypes.Length == 0) //extension method without parameter support -> prefer members
            _myScore = 0.1F;
    ...
    private float ComputeScoreForParamArray(ParameterInfo[] parameters, Type[] argTypes)
    {
        ...
        var fixedSum = ComputeSum(fixedParameters, MyFixedArgTypes);

        var paramArrayElementType = paramArrayParameter.ParameterType.GetElementType();

        var paramArraySum = 0;

        foreach (var argType in MyParamArrayArgTypes)
    ...
    public bool IsMatch(Type[] argTypes, MemberElement previous, ExpressionContext context)
    {
        ...
        if ((parameters.Length == 0) & (argTypes.Length == 0)) return true;

        // If there are no parameters but there are arguments, we cannot be a match
        if ((parameters.Length == 0) & (argTypes.Length > 0)) return false;
        ...
        if (parameters.Length == argTypes.Length && AreValidArgumentsForParameters(argTypes, parameters)) return true;
    ...
    private bool IsParamArrayMatch(Type[] argTypes, ParameterInfo[] parameters, ParameterInfo paramArrayParameter)
    {
        ...
        if (AreValidArgumentsForParameters(fixedArgTypes, fixedParameters) == false) return false;
        ...
        MemberElement previous, ExpressionContext context)
    {
        ...
        {
            if (ImplicitConverter.EmitImplicitConvert(context.ExpressionOwner.GetType(), parameters[0].ParameterType,
                    null) == false)
                return false;
        }
        else
...
internal class PropertyDictionary
{
    ...
    public PropertyDictionary Clone()
    {
        ...
        foreach (KeyValuePair<string, object> pair in _myProperties) copy.SetValue(pair.Key, pair.Value);
    ...
    public T GetValue<T>(string name)
    {
        ...
        if (_myProperties.TryGetValue(name, out value) == false) Debug.Fail($"Unknown property '{name}'");
        return (T)value;
    }
