...
internal class Utility
{
    ...
    {
        if (IsIntegralType(leftType) == false || IsIntegralType(rightType) == false) return null;
    ...
    private static bool SimpleOverloadedOperatorFilter(MemberInfo member, object value)
    {
        ...
        var nameMatch = method.IsSpecialName &&
                        method.Name.Equals((string)data["Name"], StringComparison.OrdinalIgnoreCase);

        if (nameMatch == false) return false;

        // destination type might not be known
        var destType = (Type)data["destType"];

        if (destType != null)
        {
            var returnTypeMatch = ReferenceEquals(destType, method.ReturnType);

            if (returnTypeMatch == false) return false;
        }

        ParameterInfo[] parameters = method.GetParameters();
        var argumentMatch = parameters.Length > 0 &&
                            parameters[0].ParameterType.IsAssignableFrom((Type)data["sourceType"]);

        return argumentMatch;
    }

    public static MethodInfo GetOverloadedOperator(string name, Type sourceType, Binder binder,
        params Type[] argumentTypes)
    {
        ...
        do
        {
            mi = sourceType.GetMethod(name, BindingFlags.Public | BindingFlags.Static, binder, CallingConventions.Any,
                argumentTypes, null);
            if (mi != null && mi.IsSpecialName) return mi;
